...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        code_path = utils.get_code_path() # 실행한 파일의 abs path
        default_config = {
            ...
            'BENCHMARK': 'AI_CHALLENGE',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
            'SPLIT_TO_EVAL': 'train',  # Valid: 'train', 'test', 'all'
            'INPUT_AS_ZIP': False,  # Whether tracker input files are zipped
            'PRINT_CONFIG': True,  # Whether to print current config
            'DO_PREPROC': False,#True,  # Whether to perform preprocessing (never done for MOT15)
    ...
    def _get_seq_info(self):
        seq_list = []
        seq_lengths = {}
        if self.config["SEQ_INFO"]:  # If not None, directly specify sequences to eval and their number of timesteps
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.do_preproc and self.benchmark != 'MOT15' and gt_ids.shape[0] > 0 and tracker_ids.shape[0] > 0:
                ...
                if len(invalid_classes) > 0:
                    print("invalid_classes is ", invalid_classes)
